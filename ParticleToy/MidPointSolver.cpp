...
void MidPointSolver::simulation_step(std::vector<Particle *> pVector, std::vector<Force *> fVector, std::vector<Constraint *> cVector, float dt) {
    ...
    for (int i = 0; i < pSize; i++) {
        ...
        pVector[i]->m_Velocity = DAMP * pVector[i]->m_Velocity + ma*dt; //*0.5;     //Update the legal velocity at midpoint. TODO: we should test and re-evaluate this 0.5
    ...
    for (int i = 0; i < pSize; i++) {
        ...
        pVector[i]->m_Velocity = DAMP * tmpvVector[i] + a * dt;         //Calculate the velocity at the end of the step using the forces at initial position?
